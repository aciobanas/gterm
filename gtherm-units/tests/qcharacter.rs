...
fn test_qcharacter_named_constants() {
    assert_eq!(
        QCharacter::REAL_SCALAR.tensor_order,
        Some(TensorOrder::Scalar)
    );
    assert_eq!(QCharacter::REAL_SCALAR.values_domain, ValuesDomain::Real);

    assert_eq!(
        QCharacter::COMPLEX_SCALAR.tensor_order,
        Some(TensorOrder::Scalar)
    );
    assert_eq!(
        QCharacter::COMPLEX_SCALAR.values_domain,
        ValuesDomain::Complex
    );

    assert_eq!(
        QCharacter::REAL_VECTOR.tensor_order,
        Some(TensorOrder::Vector)
    );
    assert_eq!(QCharacter::REAL_VECTOR.values_domain, ValuesDomain::Real);

    assert_eq!(
        QCharacter::COMPLEX_VECTOR.tensor_order,
        Some(TensorOrder::Vector)
    );
    assert_eq!(
        QCharacter::COMPLEX_VECTOR.values_domain,
        ValuesDomain::Complex
    );

    assert_eq!(
        QCharacter::REAL_TENSOR.tensor_order,
        Some(TensorOrder::Tensor)
    );
    assert_eq!(QCharacter::REAL_TENSOR.values_domain, ValuesDomain::Real);

    assert_eq!(
        QCharacter::COMPLEX_TENSOR.tensor_order,
        Some(TensorOrder::Tensor)
    );
    assert_eq!(
        QCharacter::COMPLEX_TENSOR.values_domain,
        ValuesDomain::Complex
    );
}
...
fn test_qcharacter_mul_scalar_absorbs_into_other_operand() {
    ...
    assert_eq!(
        QCharacter::REAL_SCALAR
            .mul(QCharacter::REAL_VECTOR)
            .tensor_order,
        Some(TensorOrder::Vector)
    );
    assert_eq!(
        QCharacter::REAL_VECTOR
            .mul(QCharacter::REAL_SCALAR)
            .tensor_order,
        Some(TensorOrder::Vector)
    );
...
fn test_qcharacter_mul_domain_combination() {
    ...
    assert_eq!(
        QCharacter::REAL_SCALAR
            .mul(QCharacter::COMPLEX_SCALAR)
            .values_domain,
        ValuesDomain::Complex
    );
    assert_eq!(
        QCharacter::COMPLEX_SCALAR
            .mul(QCharacter::COMPLEX_SCALAR)
            .values_domain,
        ValuesDomain::Complex
    );
...
fn test_qcharacter_div_by_scalar_preserves_dividend_order() {
    assert_eq!(
        QCharacter::REAL_VECTOR
            .div(QCharacter::REAL_SCALAR)
            .tensor_order,
        Some(TensorOrder::Vector)
    );
    assert_eq!(
        QCharacter::REAL_TENSOR
            .div(QCharacter::REAL_SCALAR)
            .tensor_order,
        Some(TensorOrder::Tensor)
    );
...
fn test_qcharacter_pow_zero_collapses_to_scalar() {
    assert_eq!(
        QCharacter::REAL_VECTOR.pow(0).tensor_order,
        Some(TensorOrder::Scalar)
    );
    assert_eq!(
        QCharacter::REAL_TENSOR.pow(0).tensor_order,
        Some(TensorOrder::Scalar)
    );
}
...
fn test_qcharacter_pow_nonzero_preserves_tensor_order() {
    assert_eq!(
        QCharacter::REAL_VECTOR.pow(2).tensor_order,
        Some(TensorOrder::Vector)
    );
    assert_eq!(
        QCharacter::REAL_VECTOR.pow(-1).tensor_order,
        Some(TensorOrder::Vector)
    );
    assert_eq!(
        QCharacter::REAL_SCALAR.pow(5).tensor_order,
        Some(TensorOrder::Scalar)
    );
}
...
fn test_qcharacter_pow_preserves_values_domain() {
    assert_eq!(
        QCharacter::COMPLEX_VECTOR.pow(2).values_domain,
        ValuesDomain::Complex
    );
    assert_eq!(
        QCharacter::REAL_VECTOR.pow(0).values_domain,
        ValuesDomain::Real
    );
}
