...
impl Isq {
    pub const MASS_DENSITY: QSpec =
        QSpec::new("mass_density").equation(&Div(&Term(&Isq::M), &Term(&Isq::VOLUME)));

    pub const DENSITY: QSpec = Isq::MASS_DENSITY;

    pub const SPECIFIC_VOLUME: QSpec =
        QSpec::new("specific_volume").equation(&Pow(&Term(&Isq::MASS_DENSITY), -1));
    ...
    pub const SURFACE_MASS_DENSITY: QSpec =
        QSpec::new("surface_mass_density").equation(&Div(&Term(&Isq::M), &Term(&Isq::AREA)));

    pub const SURFACE_DENSITY: QSpec = Isq::SURFACE_MASS_DENSITY;

    pub const LINEAR_MASS_DENSITY: QSpec =
        QSpec::new("linear_mass_density").equation(&Div(&Term(&Isq::M), &Term(&Isq::L)));

    pub const LINEAR_DENSITY: QSpec = Isq::LINEAR_MASS_DENSITY;

    pub const MOMENTUM: QSpec =
        QSpec::new("momentum").equation(&Mul(&Term(&Isq::M), &Term(&Isq::VELOCITY)));

    pub const WEIGHT: QSpec =
        QSpec::new("weight").equation(&Mul(&Term(&Isq::M), &Term(&Isq::ACCELERATION_OF_FREE_FALL)));

    pub const STATIC_FRICTION_FORCE: QSpec =
        QSpec::new("static_friction_force").equation(&Term(&Isq::FORCE));

    pub const STATIC_FRICTION: QSpec = Isq::STATIC_FRICTION_FORCE;

    pub const KINETIC_FRICTION_FORCE: QSpec =
        QSpec::new("kinetic_friction_force").equation(&Term(&Isq::FORCE));

    pub const DYNAMIC_FRICTION_FORCE: QSpec = Isq::KINETIC_FRICTION_FORCE;

    pub const ROLLING_RESISTANCE: QSpec =
        QSpec::new("rolling_resistance").equation(&Term(&Isq::FORCE));
    ...
    pub const DRAG_FORCE: QSpec = QSpec::new("drag_force").equation(&Term(&Isq::FORCE));

    pub const IMPULSE: QSpec =
        QSpec::new("impulse").equation(&Mul(&Term(&Isq::FORCE), &Term(&Isq::T)));
    ...
    pub const MOMENT_OF_INERTIA: QSpec = QSpec::new("moment_of_inertia")
        .equation(&Div(
            &Term(&Isq::ANGULAR_MOMENTUM),
            &Term(&Isq::ANGULAR_VELOCITY),
        ))
    ...
    pub const ANGULAR_IMPULSE: QSpec =
        QSpec::new("angular_impulse").equation(&Mul(&Term(&Isq::MOMENT_OF_FORCE), &Term(&Isq::T)));
    ...
    pub const RELATIVE_LINEAR_STRAIN: QSpec =
        QSpec::new("relative_linear_strain").equation(&Div(&Term(&Isq::L), &Term(&Isq::L)));
    ...
    pub const POISSON_NUMBER: QSpec =
        QSpec::new("Poisson_number").equation(&Div(&Term(&Isq::WIDTH), &Term(&Isq::L)));

    pub const MODULUS_OF_ELASTICITY: QSpec = QSpec::new("modulus_of_elasticity").equation(&Div(
        &Term(&Isq::NORMAL_STRESS),
        &Term(&Isq::RELATIVE_LINEAR_STRAIN),
    ));
    ...
    pub const MODULUS_OF_COMPRESSION: QSpec = QSpec::new("modulus_of_compression").equation(&Div(
        &Term(&Isq::PRESSURE),
        &Term(&Isq::RELATIVE_VOLUME_STRAIN),
    ));

    pub const BULK_MODULUS: QSpec = Isq::MODULUS_OF_COMPRESSION;

    pub const COMPRESSIBILITY: QSpec = QSpec::new("compressibility").equation(&Mul(
        &Pow(&Term(&Isq::VOLUME), -1),
        &Div(&Term(&Isq::VOLUME), &Term(&Isq::PRESSURE)),
    ));

    pub const SECOND_AXIAL_MOMENT_OF_AREA: QSpec = QSpec::new("second_axial_moment_of_area")
        .equation(&Mul(
            &Pow(&Term(&Isq::RADIAL_DISTANCE), 2),
            &Term(&Isq::AREA),
        ));

    pub const SECOND_POLAR_MOMENT_OF_AREA: QSpec = QSpec::new("second_polar_moment_of_area")
        .equation(&Mul(
            &Pow(&Term(&Isq::RADIAL_DISTANCE), 2),
            &Term(&Isq::AREA),
        ));

    pub const SECTION_MODULUS: QSpec = QSpec::new("section_modulus").equation(&Div(
        &Term(&Isq::SECOND_AXIAL_MOMENT_OF_AREA),
        &Term(&Isq::RADIAL_DISTANCE),
    ));
    ...
    pub const KINETIC_FRICTION_FACTOR: QSpec = QSpec::new("kinetic_friction_factor").equation(
        &Div(&Term(&Isq::KINETIC_FRICTION_FORCE), &Term(&Isq::FORCE)),
    );
    ...
    pub const DRAG_COEFFICIENT: QSpec = QSpec::new("drag_coefficient").equation(&Div(
        &Term(&Isq::DRAG_FORCE),
        &Mul(
            &Mul(&Term(&Isq::MASS_DENSITY), &Pow(&Term(&Isq::SPEED), 2)),
            &Term(&Isq::AREA),
        ),
    ));
    ...
    pub const DYNAMIC_VISCOSITY: QSpec = QSpec::new("dynamic_viscosity").equation(&Div(
        &Mul(&Term(&Isq::SHEAR_STRESS), &Term(&Isq::L)),
        &Term(&Isq::VELOCITY),
    ));

    pub const KINEMATIC_VISCOSITY: QSpec = QSpec::new("kinematic_viscosity").equation(&Div(
        &Term(&Isq::DYNAMIC_VISCOSITY),
        &Term(&Isq::MASS_DENSITY),
    ));

    // TODO: what is the correct equation here? (kept from the original)
    pub const SURFACE_TENSION: QSpec =
        QSpec::new("surface_tension").equation(&Div(&Term(&Isq::FORCE), &Term(&Isq::L)));

    pub const MECHANICAL_POWER: QSpec =
        QSpec::new("mechanical_power").equation(&Mul(&Term(&Isq::FORCE), &Term(&Isq::VELOCITY)));

    pub const MECHANICAL_WORK: QSpec =
        QSpec::new("mechanical_work").equation(&Mul(&Term(&Isq::FORCE), &Term(&Isq::DISPLACEMENT)));

    pub const WORK: QSpec = Isq::MECHANICAL_WORK;

    /// Differs from ISO 80000.
    pub const MECHANICAL_ENERGY: QSpec = QSpec::new("mechanical_energy").equation(&Div(
        &Mul(&Term(&Isq::M), &Pow(&Term(&Isq::L), 2)),
        &Pow(&Term(&Isq::T), 2),
    ));

    /// Differs from ISO 80000.
    pub const POTENTIAL_ENERGY: QSpec =
        QSpec::new("potential_energy").equation(&Term(&Isq::MECHANICAL_ENERGY));

    /// Differs from ISO 80000.
    pub const KINETIC_ENERGY: QSpec =
        QSpec::new("kinetic_energy").equation(&Mul(&Term(&Isq::M), &Pow(&Term(&Isq::SPEED), 2)));

    pub const MECHANICAL_EFFICIENCY: QSpec = QSpec::new("mechanical_efficiency").equation(&Div(
        &Term(&Isq::MECHANICAL_POWER),
        &Term(&Isq::MECHANICAL_POWER),
    ));
    ...
    pub const MASS_FLOW_RATE: QSpec =
        QSpec::new("mass_flow_rate").equation(&Mul(&Term(&Isq::MASS_FLOW), &Term(&Isq::AREA)));

    pub const MASS_CHANGE_RATE: QSpec =
        QSpec::new("mass_change_rate").equation(&Div(&Term(&Isq::M), &Term(&Isq::T)));

    pub const VOLUME_FLOW_RATE: QSpec =
        QSpec::new("volume_flow_rate").equation(&Mul(&Term(&Isq::VELOCITY), &Term(&Isq::AREA)));
...
const _: () = assert!(
    Isq::MASS_DENSITY
        .dims
        .const_eq(&Dims::M.const_div(&Isq::VOLUME.dims))
);
const _: () = assert!(
    Isq::SPECIFIC_VOLUME
        .dims
        .const_eq(&Isq::MASS_DENSITY.dims.pow(-1))
);
const _: () = assert!(
    Isq::MOMENTUM
        .dims
        .const_eq(&Dims::M.const_mul(&Isq::VELOCITY.dims))
);
const _: () = assert!(
    Isq::COMPRESSIBILITY.dims.const_eq(
        &Isq::VOLUME
            .dims
            .pow(-1)
            .const_mul(&Isq::VOLUME.dims.const_div(&Isq::PRESSURE.dims))
    )
);
const _: () = assert!(
    Isq::MOMENT_OF_INERTIA.dims.const_eq(
        &Isq::ANGULAR_MOMENTUM
            .dims
            .const_div(&Isq::ANGULAR_VELOCITY.dims)
    )
);
const _: () = assert!(
    Isq::KINETIC_ENERGY
        .dims
        .const_eq(&Dims::M.const_mul(&Isq::SPEED.dims.pow(2)))
);
...
const _: () = assert!(matches!(
    Isq::TORQUE.character.tensor_order,
