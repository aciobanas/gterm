...
impl Isq {
    ...
    pub const ANGULAR_MEASURE: QSpec = QSpec::new("angular_measure").equation(
        &Div(
            &Term(&Isq::ARC_LENGTH),
            &Term(&Isq::RADIUS),
        )
    ...
    pub const SOLID_ANGULAR_MEASURE: QSpec = QSpec::new("solid_angular_measure").equation(
        &Div(
            &Term(&Isq::AREA),
            &Pow(&Term(&Isq::RADIUS), 2),
        )
    );
    ...
    pub const ENERGY: QSpec = QSpec::new("energy").equation(
        &Div(
    ...
    pub const ELECTRIC_POTENTIAL: QSpec = QSpec::new("electric_potential").equation(
        &Div(
            &Term(&Isq::ENERGY),
    ...
    pub const CAPACITANCE: QSpec = QSpec::new("capacitance").equation(
        &Div(
            &Mul(&Term(&Isq::I), &Term(&Isq::T)),
    ...
    pub const LUMINOUS_FLUX: QSpec = QSpec::new("luminous_flux").equation(
        &Mul(
            &Term(&Isq::J),
            &Term(&Isq::SOLID_ANGULAR_MEASURE),
        )
    );

    pub const ILLUMINANCE: QSpec = QSpec::new("illuminance")
        .equation(&Div(&Term(&Isq::LUMINOUS_FLUX), &Term(&Isq::AREA)));

    // physical_chemistry

    pub const CATALYTIC_ACTIVITY: QSpec = QSpec::new("catalytic_activity")
        .equation(&Div(&Term(&Isq::N), &Term(&Isq::T)));
    ...
    pub const DOSE_EQUIVALENT: QSpec = QSpec::new("dose_equivalent").equation(
        &Mul(
    ...
    pub const POWER: QSpec = QSpec::new("power").equation(
        &Div(
            &Mul(&Term(&Isq::M), &Pow(&Term(&Isq::L), 2)),
            &Pow(&Term(&Isq::T), 3),
        )
    ...
    pub const ENERGY_DENSITY: QSpec = QSpec::new("energy_density").equation(
        &Div(
            &Term(&Isq::M),
